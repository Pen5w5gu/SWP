    ...
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion"
        ...
        <li class="nav-item">
            <a class="nav-link collapsed" href="#" data-toggle="collapse"
               data-target="#listStudent" aria-expanded="true" aria-controls="collapsePages">
                <i class="fas fa-fw fa-folder"></i>
                <span>List student</span>
            </a>

            <div id="listStudent" class="collapse" aria-labelledby="headingPages"
                 data-parent="#accordionSidebar">
                <div class="bg-white py-2 collapse-inner rounded">
                    <h6 class="collapse-header">in class:</h6>
                    <div class="collapse-divider"></div>
                    <c:choose>
                        <c:when test="${not empty classes}">
                            <c:forEach var="myclass" items="${classes}">
                                <form action="showstudentinclass" method="GET" onsubmit="return submitForm(${myclass.class_name})">
                                    <a href="#" class="collapse-item" onclick="this.parentNode.submit();">Class ${myclass.class_name}</a>
                                    <input type="hidden" name="classname" value="${myclass.class_name}">
                                </form>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
            ...
            <div id="collapsePages" class="collapse" aria-labelledby="headingPages"
                ...
                <div class="bg-white py-2 collapse-inner rounded">
                    ...
                    <c:choose>
                        <c:when test="${not empty classes}">
                            <c:forEach var="myclass" items="${classes}">
                                <form action="showproject" method="GET" onsubmit="return submitForm(${myclass.class_name})">
                                    <a href="#" class="collapse-item" onclick="this.parentNode.submit();">Class ${myclass.class_name}</a>
                                    <input type="hidden" name="classname" value="${myclass.class_name}">
                                </form>
                            </c:forEach>
