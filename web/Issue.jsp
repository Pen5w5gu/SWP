    ...
    <body id="page-top">
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                // Danh sách để lưu trữ các ID của issue đã được chọn
                var selectedIssues = [];
                ...
                checkboxes.forEach(function (checkbox) {
                    checkbox.addEventListener('click', function () {
                        ...
                        if (this.checked) {
                            // Nếu được chọn, thêm ID của issue vào danh sách
                            selectedIssues.push(this.id);
                        } else {
                            // Nếu không được chọn, loại bỏ ID của issue khỏi danh sách
                            var index = selectedIssues.indexOf(this.id);
                            if (index !== -1) {
                                selectedIssues.splice(index, 1);
                            }
                        }

                        // Hiển thị các button nếu có ít nhất một issue được chọn
                        displayActionIssues();
                    });
                });

                // Hàm để hiển thị số lượng issue đã được chọn và các button tương ứng
                function displayActionIssues() {
                    var issueCountContainer = document.getElementById('issueCount');
                    var uncheckAllButton = document.getElementById('uncheckAllButton');
                    var sendButton = document.getElementById('sendButton');

                    // Hiển thị số lượng issue đã được chọn
                    issueCountContainer.textContent = 'Số lượng issue đã chọn: ' + selectedIssues.length;

                    // Kiểm tra nếu có ít nhất 1 issue được chọn thì hiển thị nút gửi và nút bỏ tích tất cả, ngược lại ẩn nút gửi và nút bỏ tích tất cả
                ...
                // Hàm để bỏ tích tất cả các issue đã chọn
                function uncheckAllIssues() {
                    ...
                    // Xóa danh sách các issue đã chọn và cập nhật hiển thị
                    selectedIssues = [];
                    displayActionIssues();
                }
                ...
                uncheckAllButton.addEventListener('click', uncheckAllIssues);
        ...
        <script>
            ...
            function Sendata(issueId, newIssueTypeId) {
                ...
                // Gửi yêu cầu POST với dữ liệu issueId và newIssueTypeId
                xhr.send("issueId=" + issueId + "&newIssueTypeId=" + newIssueTypeId);
            }


            function sendSelectedIssues() {
                // Lấy chuỗi giá trị đã chọn
                var selectedValues = getSelectedCheckboxValues();

                // Đường dẫn tới servlet
                var servletURL = 'select_AssignIssue';

                // Tạo URL với tham số selectedIssues
                var urlWithParams = servletURL + '?selectedIssues=' + encodeURIComponent(selectedValues);
            ...
            <div id="content-wrapper" class="d-flex flex-column">
                ...
                <div id="content">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span class="font-weight-bold">${noti.user_name} ${noti.notification} 
                                                            <c:forEach items="${issues}" var="issue"> 
                                                                <c:if test="${issue.idIssue == noti.id_issue}">
                                                                    ${issue.issueName}
                                                                </c:if>
                                                            </c:forEach>
                                                        </span>
                    ...
                    <div class="container-fluid">
                        ...
                        <!-- Page Heading -->
                        <h1 class="h3 mb-2 text-gray-800">Issue</h1>
                        <p class="mb-4">List of issue</p>
                        ...
                        <div class="card shadow mb-4 ">
                            <div class="card-header d-sm-flex align-items-center justify-content-between mb-4">
                                ...
                                <c:if test="${session.getRole_project() == 'TL'}">
                                    <a href="add_issue" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                                        <i class="fa-solid fa-plus"></i> New issue
                                    </a>
                                </c:if>
                            </div>
                            <div id="actionButtons" style="display:none; margin-left: 20px;">
                                <p id="issueCount">Số lượng issue đã chọn: 0</p>
                                <button id="uncheckAllButton" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm ">Bỏ tích tất cả</button>
                                <button id="sendButton" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm " onclick="sendSelectedIssues()">Gửi danh sách ID đã chọn</button>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <form action="comment" method="POST">
                                                ...
                                                <tbody>
                                                    ...
                                                    <c:forEach items="${issues}" var="issue" varStatus="loop">
                                                        <tr>
                                                            ...
                                                            <td>${issue.id}</td>
                                                            <td>
                                                                <a href="${pageContext.request.contextPath}/comment_issue?id=${issue.id}">${issue.name}</a>
                                                            </td>
                                                            <td>${issue.content}</td>
                                                            ...
                                                            <td>
                                                                <select id="status-${issue.id}" 
                                                                        ...
                                                                        name="status"
                                                                        onchange="changeStatusIssue('${issue.id}', this.value)">
                                                                    <option value="NEW">NEW</option>
                                                                    <option value="IN-PROGRESS">IN-PROGRESS</option>
                                                                    <option value="CLOSED">CLOSED</option>
                                                                </select>
                                                                <script>
                                                                    let status${issue.id} = "${issue.status}";
                                                                    document.getElementById("status-${issue.id}").value = status${issue.id};
                                                                </script>
                                                            </td>
                                                            <td>
                                                                <a class="btn btn-primary" href="${pageContext.request.contextPath}/edit_issue?id=${issue.id}">Edit</a>
                                                                <a class="btn btn-danger" href="${pageContext.request.contextPath}/delete_issue?id=${issue.id}">Delete</a>
                                                            </td>
                                                        </tr>
                                                    </c:forEach>
                                                </tbody>
                                        ...
                                        <script>
                                            async function changeStatusIssue(issueId, newStatus) {
                                                const formData = new URLSearchParams();
                                                formData.append("issueId", issueId);
                                                formData.append("status", newStatus);

                                                await fetch("${pageContext.request.contextPath}/change_status_issue", {
                                                    method: "POST",
                                                    body: formData
                                                });
                                            }
